...
int main(void)
{
    ...
    fprintf(gp, "set cntrparam levels incremental -1.0,1.0,4.0\n");
...
void apply_bc(double psi[][NY+1], double omega[][NY+1])
{
    ...
    for (int i = 0; i <= NX; i++) {
        if (is_solid[i][NY]) {
            ...
            if (!is_solid[i][NY-1]) {
                omega[i][NY] =
                    -2.0*(psi[i][NY-1] - psi[i][NY])/(dy*dy);//すべり壁面( = 渦無し流れ)なので，EF上ではω = 0 (8.31)
            } else {
                omega[i][NY] = 0.0;
            }
        }
    ...
    //BCの渦度の更新がない(8.28), ⊿y -> ⊿x

    //点Bの渦度の更新がない(8.29)

    //仮にAB, BCでの渦度を計算していたとして，ここで全て渦度を0に初期化してしまっている
    for (int i = 0; i <= NX; i++) {
        for (int j = 0; j <= NY; j++) {
            if (is_solid[i][j]) {
                psi[i][j]   = 0.0;
